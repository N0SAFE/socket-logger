...
import Cluster from '../../../server/utils/Cluster'

describe('Cluster', () => {
  const servers: Array<Server> = []
  const httpServer = Cluster.createHttpServer()
  ...
  it('should create a cluster with default options', () => {
    const cluster = new Cluster({ port: 65100 })
    servers.push(cluster)
    expect(cluster.port).to.equal(65100)
    expect(cluster.p).to.equal('/')
    expect(cluster.servers.size).to.equal(0)
    expect(cluster.isOpened).to.be.true
  })

  it('should create a cluster with custom options', () => {
    const cluster = new Cluster({ port: 65100, path: '/test' })
    servers.push(cluster)
    expect(cluster.port).to.equal(65100)
    expect(cluster.p).to.equal('/test')
    expect(cluster.servers.size).to.equal(0)
    expect(cluster.isOpened).to.be.true
  })

  it('should create a cluster using an httpServer', () => {
    const cluster = new Cluster({ httpServer: httpServer, path: '/test' })
    servers.push(cluster)
    expect(cluster.port).to.equal(65200)
    expect(cluster.p).to.equal('/test')
    expect(cluster.servers.size).to.equal(0)
    expect(cluster.isOpened).to.be.true
  })
  ...
  it('should add a server to the cluster', () => {
    const cluster = new Cluster({ httpServer: httpServer })
    const server = cluster.createServer({ port: 65201 })
    servers.push(cluster, server)
    expect(server).to.not.be.undefined
    expect(cluster.servers.size).to.equal(1)
    expect(cluster.serverExists({ port: 65201 })).to.be.true
    expect(cluster.serverExists({ port: 65201, path: '/socket.io' })).to.be
      .false
    expect(cluster.serverExists({ port: 65201 + 1 })).to.be.false
  })
  ...
  it('should add a server to the cluster using an httpServer', () => {
    ...
    expect(server).to.not.be.undefined
    expect(cluster.servers.size).to.equal(1)
    expect(cluster.serverExists({ httpServer: httpServer })).to.be.true
    expect(cluster.serverExists({ port: httpServer.port as number })).to.be.true
    expect(
      cluster.serverExists({
        port: httpServer.port as number,
        path: '/socket.io',
      }),
    ).to.be.false
    expect(cluster.serverExists({ port: (httpServer.port as number) + 1 })).to
      .be.false
  })
  ...
  it('should redirect a client to a server', (done) => {
    ...
    cluster.onConnection((connection) => {
      expect(connection.server).to.equal(cluster)
      expect(cluster.connections.size).to.equal(1)
    })
    ...
    const promise2 = new Promise((resolve, reject) => {
      server.onConnection(() => {
        try {
          ...
          expect(client.info?.port).to.equal(httpServer.port)
          expect(client.info?.path).to.equal('/server1')
          resolve('ok')
        } catch (e) {
          reject(e)
        }
    ...
    ;(async () => {
      ...
      const promise1 = new Promise((resolve) => {
        client1.onRedirect((info) => {
          ...
          expect(info.path).to.equal('/server1')
          resolve('ok')
        })
      })
      ...
      const promise2 = new Promise((resolve, reject) => {
        server1.onConnection(() => {
          try {
            expect(server1.connections.size).to.equal(1)
            expect(client1.info?.port).to.equal(httpServer.port)
            expect(client1.info?.path).to.equal('/server1')
            resolve('ok')
          } catch (e) {
            reject(e)
          }
      ...
      const promise3 = new Promise((resolve) => {
        client2.onRedirect((info) => {
          expect(info.type).to.equal('redirected')
          expect(info.path).to.equal('/server2')
          resolve('ok')
        })
      })
      ...
      const promise4 = new Promise((resolve, reject) => {
        server2.onConnection(() => {
          try {
            expect(server2.connections.size).to.equal(1)
            expect(client2.info?.port).to.equal(httpServer.port)
            expect(client2.info?.path).to.equal('/server2')
            resolve('ok')
          } catch (e) {
            reject(e)
          }
